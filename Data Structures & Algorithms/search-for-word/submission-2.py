class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        def backtrack(current, row, col, visited):
            ...
            if row-1>0 and board[row-1][col]==word[current] and visited[row-1][col]==0:
                    visited[row-1][col]==1             
                    backtrack(current+1, row-1, col, visited)
                    visited[row-1][col]==0

            if col-1>0 and board[row][col-1]==word[current] and visited[row][col-1]==0:
                    visited[row][col-1]==1
        ...
        a=[0]*self.ncols
        visited=[a for _ in range(self.nrows)]
        print(visited)
